...
def generate_schedules():
    ...
    specialties = ["cardiology", "dentist", "general_practice",
    ...
    providers = ["Dr. John Smith", "Dr. Jane Doe", "Dr. Jim Beam",
                 "Dr. Jill Hill", "Dr. Jack Daniels", "Dr. John Doe", "Dr. Jane Smith"]
    working_hours = {"start_time": "9:00", "end_time": "17:00"}
    appointment_duration = 30
    appointment_buffer = 15
    dates = ["2025-03-19", "2025-03-20", "2025-03-21",  "2025-03-24",
        ...
        for location in locations:
            ...
            for provider in providers:
                start_time = datetime.strptime(
                    working_hours["start_time"], "%H:%M")
                end_time = datetime.strptime(
                    working_hours["end_time"], "%H:%M")
                ...
                while current_time + timedelta(minutes=appointment_duration + appointment_buffer) < end_time:
                    for date in dates:
                        appointments.append({
                            ...
                            "end_time": (current_time + timedelta(minutes=appointment_duration)).strftime("%H:%M")
                        })
                        current_time += timedelta(
                            minutes=appointment_duration + appointment_buffer)

            print(
                f"Inserting {len(appointments)} appointments for {specialty} in {location} with {provider}")
...
def get_schedules(condition):
    ...
    res = schedule_collection.find({"specialty": {"$eq": "surgeon"}, "timestamp": {
                                   "$gt": datetime.strptime("2025-03-25T00:00:00Z", "%Y-%m-%dT%H:%M:%S%z"), "$lte": datetime.strptime("2025-03-26T00:00:00Z", "%Y-%m-%dT%H:%M:%S%z")}})
    print("Result:")
    print(res)
    for schedule in res:
        print("--------------------------------")
